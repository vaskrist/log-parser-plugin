...
public abstract class LogParserTestCase {
    ...
    protected FreeStyleProject setupFreeStyleProject(boolean broken) throws URISyntaxException, IOException {
        ...
        if (broken) {
            logParser = new LogParserPublisher(false, false, false, null, false, null);
        } else {
            logParser = new LogParserPublisher(true, true, true,
                    new File(getClass().getResource("/hudson/plugins/logparser/LogParserTestCase/temp.rules").toURI())
                            .getAbsolutePath(),
                    false, null);
        }
        ...
        if (Functions.isWindows()) {
            ...
            project.getBuildersList().add(
                    new Shell("echo \"This is a LogParserTest\" && echo \"INFO\" && echo \"WARN\" && echo \"ERROR\""));
        }
